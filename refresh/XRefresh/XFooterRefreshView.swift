...
class XFooterRefreshView: UIView {
    ...
    override func observeValueForKeyPath(keyPath: String?, ofObject object: AnyObject?, change: [String : AnyObject]?, context: UnsafeMutablePointer<Void>) {
        ...
        {
            var offy:CGFloat = scrollView!.contentSize.height
            
            if self.scrollView!.contentSize.height < self.scrollView!.frame.size.height
            {
                if scrollView is UICollectionView
                {
                    offy = self.scrollView!.frame.size.height
                }
                else
                {
                    self.scrollView!.contentSize.height = self.scrollView!.frame.size.height
                    offy = scrollView!.contentSize.height
                }
                
            }
            
            self.frame=CGRectMake(0, offy, scrollView!.frame.size.width, height)
    ...
    func endRefresh()
    {
        ...
        dispatch_after(popTime, dispatch_get_main_queue(), { () -> Void in
            if self.state != .End{self.state = .Normal}
        })
    }
